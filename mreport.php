    ...
    if (confirm("Do you want to Print?!") == true) {
           ...
           var t1 = document.getElementById('tab1');
				...
				//alert(t1);
							...
							printWindow.document.write('<html><head><title>Date Wise Report</title><style>input{border:none;} table{width:1000px !important;}table tr th{border-bottom:1px solid black;} table tr td{text-align:center;}table tr #th1,#th2{border-top:1px solid black;}#but1{display:none;}</style>');
							...
							printWindow.document.write('<div style="font-size:24px;text-align:center;font-weight:bold;border-bottom:1px dashed;border-top:1px dashed;">Date Wise Report</div>');
...
<div class="row">
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> Date Wise Reports</h2>
   ...
   <form action='mreport.php' method='post' name='form1'>   
...
<td><input type='submit' name='sub2'  onclick="return chk1();"  value="View" class="btn btn-info"></td>
	 ...
	 $grp="";	  
	  ...
	  {
	  $sql2=mysql_query("select * from order_details where date1 between '$_POST[date1]' and '$_POST[date2]' and type1='table'");
	  }
	  else
	  {
	  $sql2=mysql_query("select * from order_details where date1 between '$_POST[date1]' and '$_POST[date2]' and type1='direct'");
	  }
     ...
     $sum=0;
	  ...
	  {
	  $sql2=mysql_query("select * from order_details where type1='table' and (EXTRACT(MONTH from date1)='$_POST[month1]' and EXTRACT(YEAR from date1)='$_POST[year1]') order by sno desc") or die(mysql_error());
	  }
	  else
	  {
	  $sql2=mysql_query("select * from order_details where type1='direct' and (EXTRACT(MONTH from date1)='$_POST[month1]' and EXTRACT(YEAR from date1)='$_POST[year1]') order by sno desc") or die(mysql_error());
	  }
