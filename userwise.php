    ...
    if (confirm("Do you want to Print?!") == true) {
           ...
           var t1 = document.getElementById('tab1');
				...
				//alert(t1);
							...
							printWindow.document.write('<html><head><title>User Wise Report</title><style>input{border:none;} table{width:1000px !important;}table tr th{border-bottom:1px solid black;} table tr td{text-align:center;}table tr #th1,#th2{border-top:1px solid black;}#but1{display:none;}</style>');
							printWindow.document.write('</head><body >');
							
							printWindow.document.write('<div style="text-align:center;border:1px dashed #eee;padding:10px;">')
							printWindow.document.write('<div style="font-size:36px;text-align:center;font-family:brush script;">Grab-n-Eat</div>');
							...
							printWindow.document.write('<div style="font-size:24px;text-align:center;font-weight:bold;border-bottom:1px dashed;border-top:1px dashed;">User Wise Report</div>');
 ...
 else if(document.form1.date2.value=="")
{
 ...
 else if(document.form1.user1.value=="")
{
...
return true;
  ...
  function chk1()
 {
 if(document.form1.group2.value=="select")
 {
 alert("Please Select Any Order");
 ...
 else if(document.form1.year1.value=="")
{
 ...
 else if(document.form1.user2.value=="")
{
   alert("Please Select User");
...
<div class="row">
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> User Wise Reports</h2>
   ...
   <form action='userwise.php' method='post' name='form1'>   
...
<th>From Date<select name='date1' class="form-control">
...
<th>To Date<select name='date2' class="form-control">
...
<th>
User<select name='user1' class="form-control">
<option value="">Select User</option>
		<?php
		$sql1=mysql_query("select distinct(user) from sainaam");
		while($arr=mysql_fetch_array($sql1))
		{
		echo "<option value='$arr[user]'>$arr[user]</option>";
...
<table  class="table table-striped table-bordered bootstrap-datatable datatable responsive">
<tr>
<th>Order<select name='group2' class="form-control">
<option value="select">Select</option>
<option value="Table">Table</option>
<option value="Direct">Direct</option>
</select>
</th>
<th>Month<select name='month1' class="form-control">
...
<th>Year<select name='year1' class="form-control">
...
<th>
User<select name='user2' class="form-control">
<option value="">Select User</option>
		<?php
		$sql1=mysql_query("select distinct(user) from sainaam");
		while($arr=mysql_fetch_array($sql1))
...
<td><input type='submit' name='sub2'  onclick="return chk1();"  value="View" class="btn btn-info"></td>
	 ...
	 $grp="";	  
	  ...
	  {
	  $sql2=mysql_query("select * from order_details where date1 between '$_POST[date1]' and '$_POST[date2]' and type1='table' and user='$_POST[user1]'") or die(mysql_error());
	  }
	  else
	  {
	  $sql2=mysql_query("select * from order_details where date1 between '$_POST[date1]' and '$_POST[date2]' and type1='direct' and user='$_POST[user1]'") or die(mysql_error());
	  }
     ...
     $sum=0;
	  ...
	  {
	  if($_POST['group2']=='Table')
	  {
	  $sql2=mysql_query("select * from order_details where type1='table' and user='$_POST[user2]' and (EXTRACT(MONTH from date1)='$_POST[month1]' and EXTRACT(YEAR from date1)='$_POST[year1]') order by sno desc ") or die(mysql_error());
	  }
	  else
	  {
	  $sql2=mysql_query("select * from order_details where type1='direct' and user='$_POST[user2]' and (EXTRACT(MONTH from date1)='$_POST[month1]' and EXTRACT(YEAR from date1)='$_POST[year1]') order by sno desc ") or die(mysql_error());
	  }
	...
	echo "<tr><th style='width:200px !important;'>Order </th><th style='text-align:left;'>$grp Order</th></tr>";	
		...
		echo "<tr><th>User </th><th style='text-align:left;'>$_POST[user1]</th></tr>";			
		}
		else
		{
		...
		echo "<tr><th>User </th><th style='text-align:left;'>$_POST[user2]</th></tr>";			
		}
